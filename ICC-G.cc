...
int IntroCC_G::flow_id_counter = 0;

double IntroCC_G::current_timestamp( void ){
  return cur_tick;
}
...
void IntroCC_G::GoertzelInit(double fs, int N)
{
	if (fs <= 0.0 || N < 4) return;
	g_fs = fs;
	g_N  = N;

	// Band centred on f_center = 1/(5*min_rtt)  (min_rtt in ms → Hz)
	double rttmin_s = min_rtt / 1000.0;
	double f_center;
	if (rttmin_s > 0.0 && rttmin_s < 1.0e3)
		f_center = 1.0 / (5.0 * rttmin_s);
	else
		f_center = 5.0 * fs / (double)N;

	const double G_BAND = 3.0;
	double f_low  = std::max(1.5,      f_center / G_BAND);
	double f_high = std::min(fs / 2.0, f_center * G_BAND);
	if (f_high <= f_low) f_high = std::min(fs / 2.0, f_low * 2.0);
	if (f_high <= f_low) f_high = f_low * 2.0;

	for (int k = 0; k < G_BINS; ++k)
	{
		double frac = (G_BINS > 1) ? (double)k / (G_BINS - 1) : 0.0;
...
void IntroCC_G::GoertzelFeed(double qd_sample, double cwnd_sample)
{
...
int IntroCC_G::find_fm()
{
	...
	for (int i = 0; i < n; ++i) { dc_qd += P_qd[i]; dc_cw += P_cwnd[i]; }
	double mean_qd = dc_qd / n;
	double mean_cw = dc_cw / n;

	// ── Bootstrap: first cycle replays buffer with mean removed ──
	if (!g_initialized)
	{
		g_dcQd = mean_qd;
		g_dcCw = mean_cw;
		GoertzelInit(fs, n);
		for (int i = 0; i < n; ++i)
			GoertzelFeed(P_qd[i], P_cwnd[i]);
	}

	// ── Extract Goertzel power at each candidate bin ─────────────
	double best_qd_power = -1.0;
	int    best_k        = 0;

	double qd_dc_amp = std::fabs(dc_qd);
	...
	for (int k = 0; k < G_BINS; ++k)
		similarity += std::fabs(rtt_amp[k] - cw_amp[k]);
	similarity /= n;

	double simiD = std::abs((double)(1.0 * (rttInc - cwndInc))) / n;

#ifdef LDEBUG
	std::cerr << "Interval arrive: " << current_timestamp()
	          << " P_max " << P_max << " P_min " << P_min
	          << " similarity: " << similarity
	          << " simiD: " << simiD
	          << " rttInc " << rttInc << " cwndInc " << cwndInc;
#endif

	double fm_th = 1.0 / (20.0 * min_rtt / 1000.0);
	if (similarity > 0.02 || simiD > 0.7) return -1;
	if (fm > 0 && pre_fm > 0 && std::abs(pre_fm - fm) <= std::min(5.0 * fm_th, 2.0))
		return 1;
	return 0;
}
...
void IntroCC_G::update_lamda(double Qamp,double gain,Time rtt){//bool pkt_lost __attribute((unused)), double cur_rtt) {
...
double IntroCC_G::randomize_intersend(double intersend) {
...
void IntroCC_G::update_intersend_time(Time RTTinst) {
...
void IntroCC_G::onACK(int ack, 
  ...
  if (probe_cycle_start < cur_time)
  {
	  ...
	  GoertzelFeed(rtt_measured, _the_window);
  ...
  if (probe_cycle_start+cycle*1000 < cur_time)
  {
	  ...
	  // ── Re-arm Goertzel for the next cycle ─────────────────────────
	  if (!P_qd.empty())
	  {
		  double mq = 0.0, mc = 0.0;
		  for (size_t i = 0; i < P_qd.size(); ++i) { mq += P_qd[i]; mc += P_cwnd[i]; }
		  g_dcQd = mq / P_qd.size();
		  g_dcCw = mc / P_cwnd.size();
		  GoertzelInit(P_qd.size() / cycle, (int)P_qd.size());
	  }
	  else
	  {
		  g_initialized = false;
...
void IntroCC_G::onPktSent(int seq_num) {
  double cur_time = current_timestamp();
...
void IntroCC_G::close() {
}

void IntroCC_G::onDupACK() {

}

void IntroCC_G::onTimeout() {

}

void IntroCC_G::interpret_config_str(string config, string Bd_config, string Rc_config) { //config
